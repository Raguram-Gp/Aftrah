...
import type { AdvancePayment, PaymentMode } from '../../types';
...
export const EditAdvanceModal: React.FC<EditAdvanceModalProps> = ({
  ...
  const { updateAdvance } = useSiteManager();
  ...
  useEffect(() => {
    if (advance) {
      ...
      setPaymentMode(advance.paymentMode || 'Bank Transfer');
      setAmount(advance.amount);
      setReferenceNotes(advance.referenceNotes || '');
      setReceivedBy(advance.receivedBy || '');
    }
  }, [advance]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateAdvance(siteId, advance.id, {
      date,
      paymentMode,
      ...
      <div className="modal-card" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-header">
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <div>
              <h2 className="modal-title">Edit Advance Entry</h2>
