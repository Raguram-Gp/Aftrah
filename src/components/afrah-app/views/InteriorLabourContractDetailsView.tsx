...
export const InteriorLabourContractDetailsView: React.FC<InteriorLabourContractDetailsViewProps> = ({
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    ...
    if (!isValidDate(addDate) || !addWorkType.trim() || isNaN(days) || isNaN(salary) || isNaN(total)) return;
  ...
  const handleSaveEdit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!editingEntry || !isValidDate(editDate)) return;
              ...
              {filteredEntries.length > 0 && (
                <tfoot>
                  <tr style={{ background: 'rgba(255, 255, 255, 0.02)', fontWeight: 700 }}>
                    ...
                    <td>-</td>
                    <td style={{ color: 'var(--text-primary)' }}>{formatINR(paidAmount)}</td>
                    <td />
                  </tr>
                </tfoot>
              )}
